...
const {adhocExecTask, configurationErrorTask} = __webpack_require__(614);
const {NOOP, appendTaskOptions, asArray, filterArray, filterPrimitives, filterString, filterStringOrStringArray, filterType, folderExists, getTrailingOptions, trailingFunctionArgument, trailingOptionsArgument} = __webpack_require__(532);
...
const {hashObjectTask} = __webpack_require__(521);
const {initTask} = __webpack_require__(49);
const {logTask, parseLogOptions} = __webpack_require__(525);
const {mergeTask} = __webpack_require__(508);
const {moveTask} = __webpack_require__(971);
const {pullTask} = __webpack_require__(410);
const {pushTagsTask, pushTask} = __webpack_require__(938);
...
const {straightThroughStringTask} = __webpack_require__(614);
...
const getDirectories_1 = __webpack_require__(817);
const checkMainGitPath_1 = __webpack_require__(541);
const execTerraform_1 = __webpack_require__(610);
const prepareTerragrunt_1 = __webpack_require__(190);
...
const execTerragrunt = (processCwd, apply, log) => {
...
const spawnSyncTerragrunt = (options, log) => {
    const run = child_process_1.spawnSync('terragrunt', options);
...
Object.defineProperty(exports, "__esModule", { value: true });
exports.prepareTerragrunt = void 0;
const fs = __importStar(__webpack_require__(747));
const path_1 = __importDefault(__webpack_require__(622));
...
const prepareTerragrunt = (processCwd, workingDirectory, apply, log) => {
    ...
    for (const batch of dirBatches) {
        ...
        execTerragrunt_1.execTerragrunt(workingDirectory, apply, log);
        removeFileFromDir(workingDirectory, batch, "terragrunt.hcl");
    }
    return true;
};
exports.prepareTerragrunt = prepareTerragrunt;
function addFileInDirs(workingDir, dirList, fileName) {
    for (const dir of dirList) {
        const dirPath = path_1.default.join(workingDir, dir);
        const dirFile = path_1.default.join(dirPath, fileName);
        fs.writeFileSync(dirFile, '');
        console.log(`File "${fileName}" was added in "${dirPath}"`);
    }
}
function removeFileFromDir(workingDir, dirList, fileName) {
    for (const dir of dirList) {
        const filePath = path_1.default.join(workingDir, dir, fileName);
        fs.unlinkSync(filePath);
        console.log(`File "${fileName}" was deleted from "${path_1.default.join(workingDir, dir)}"`);
    }
}
function getDirInBatches(dir, batchSize) {
    const dirList = fs.readdirSync(dir)
        .filter(item => fs.statSync(path_1.default.join(dir, item)).isDirectory());
    const filteredDirList = filterDirList(dirList);
    const batch = [];
...
function filterDirList(dirList) {
